...
class LemonadeConversationAgent(AbstractConversationAgent):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self.model: str = data[CONF_MODEL]
        ...
        self.tool_iter_limit: int = int(options.get(CONF_TOOL_ITER_LIMIT, 4))
        ...
        _LOGGER.debug(
            "LemonadeConversationAgent init: model=%s endpoint=%s tools=%s stream=%s",
            self.model, self.endpoint, self.enable_tools, self.stream
        )
    ...
    def supported_languages(self) -> list[str]:
        # Declaramos explícitamente idiomas; evita problemas en el frontend.
        return ["es", "en"]
    ...
    async def async_process(self, user_input: ConversationInput) -> ConversationResult:
        ...
        def _set_speech(resp: intent.IntentResponse, text: str) -> None:
            # Compatibilidad con distintas versiones de HA
            if hasattr(resp, "async_set_speech_plain"):
                ...
                # Fallback: setear atributo interno (último recurso)
                try:
                    resp.response = {"speech": {"plain": {"speech": text or ""}}}
        ...
        try:
            ...
            _LOGGER.debug(
                "Process start: conv_id=%s lang=%s text=%s device_id=%s",
                conv_id, language, text, getattr(user_input, "device_id", None)
            )
            ...
            tools = build_tools_schema() if self.enable_tools else None

            # Si hay tools, desactiva streaming para permitir function calling robusto
            use_stream = self.stream and not self.enable_tools and self.endpoint == DEFAULT_ENDPOINT
            ...
            while True:
                ...
                if "choices" in resp:
                    ...
                    messages.append(
                        {"role": "assistant", "content": assistant_text or "", "tool_calls": tool_calls}
                    )
            ...
            _LOGGER.debug("Process end OK: conv_id=%s text=%s", conv_id, final_text)
